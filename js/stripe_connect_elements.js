/**
 * @file
 * JavaScript for the Stripe Elements integration with support for direct charges.
 */
...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.stripeConnectElements = {
    attach: function (context, settings) {
      ...
      // Get the connected account ID if available
      const connectedAccountId = drupalSettings.stripeConnect.connectedAccountId || null;
      
      // Initialize Stripe with the publishable key
      let stripeOptions = {};
      if (connectedAccountId) {
        // If a connected account ID is provided, use it to initialize Stripe
        // This allows the Elements to be created directly in the connected account's context
        stripeOptions.stripeAccount = connectedAccountId;
      }
      
      const stripe = Stripe(drupalSettings.stripeConnect.publishableKey, stripeOptions);
      ...
      if (cardElementContainer) {
        ...
        if (form) {
          $(form).once('stripe-connect-form').on('submit', function (event) {
            ...
            // Create a payment method that will work with connected accounts
            ...
            .then(function (result) {
              if (result.error) {
                ...
                // Also save the connected account ID if available, for cross-account usage
                if (connectedAccountId) {
                  const accountInput = document.createElement('input');
                  accountInput.type = 'hidden';
                  accountInput.name = 'stripe_connect_account_id';
                  accountInput.value = connectedAccountId;
